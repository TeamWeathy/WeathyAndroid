...
internal class Snowflake(
    private val randomizer: Randomizer, private val params: Params
) {
    ...
    internal fun reset(positionY: Double? = null) {
        ...
        if (params.image != null) {
            bitmap = Bitmap.createScaledBitmap(params.image, (size * params.imageRatio).roundToInt(), size, false)
        }

        val speed =
            ((size - params.sizeMinInPx).toFloat() / (params.sizeMaxInPx - params.sizeMinInPx) * (params.speedMax - params.speedMin) + params.speedMin)
        val angle =
            toRadians(randomizer.randomDouble(params.angleMax) * if (params.sameDirection) 1 else randomizer.randomSignum())
